...
export const addNewMeasurementUnit = async (req, res) => {
  ...
  try {
    const existingUnit = await MeasurementUnitsReferenceModel.findOne({
      unitName,
    });
    if (existingUnit) {
      return res
        .status(400)
        .send({ message: 'Such measurement unit already exists' });
    }

    const doc = new MeasurementUnitsReferenceModel({
      unitName,
      unitDescription,
    });
    await doc.save();
...
export const getMeasurementUnits = async (req, res) => {
  ...
  try {
    ...
    if (unitName) {
      const measurementUnits = await MeasurementUnitsReferenceModel.find({
        unitName: { $regex: new RegExp(unitName, 'i') },
      });
      if (!measurementUnits) {
        return res.status(404).json({
          message: 'No measurement found with the requested parameters.',
        });
      }
